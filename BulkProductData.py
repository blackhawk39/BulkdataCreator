...
# -*- coding: utf-8 -*-
"""
Created on Wed Jul 21 13:07:49 2021
...
def get_database():
    ...
    # CONNECTION_STRING = "mongodb+srv://%s:%s@cluster0.yotf1.mongodb.net/TestBulkData?retryWrites=true&w=majority" % (username, password)

    client = MongoClient("mongodb+srv://%s:%s@cluster0.yotf1.mongodb.net/TestBulkData?retryWrites=true&w=majority" % (username, password))

    # Create the database for our example (we will use the same database throughout the tutorial
    return client['TestBulkData']
...
col.insert_many(flist)
df1=pd.DataFrame(flist)
df1.to_csv('cfile.csv')
print("clothes done")
...
# col.insert_many(flist)
df2=pd.DataFrame(flist)
df2.to_csv('bfile.csv')
print("books done")

flist=data_add(gname, gvar, gilist, "grocery", 200000, 300000)
# col.insert_many(flist)
df3=pd.DataFrame(flist)
print("grocery done")
df3.to_csv('gfile.csv')

flist=data_add(fname, fvar, filist, "food", 300000, 400000)
# col.insert_many(flist)
df4=pd.DataFrame(flist)
df4.to_csv('ffile.csv')
print("food done")

flist=data_add(mname, mvar, milist, "medical", 400000, 500000)
# col.insert_many(flist)
df5=pd.DataFrame(flist)
df5.to_csv('mfile.csv')
print("medical done")

# df=pd.concat([df1,df2,df3,df4,df5]).reset_index(drop=True)

# response_json = ("{ \"response_json\":" + str(flist)+ "}")
# with open('lastfilex.json', 'w') as fout:
#     json.dump(flist, fout)